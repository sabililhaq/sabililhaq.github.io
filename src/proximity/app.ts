...
import { parseProximityJson, serializeProximity, type ProximityFile } from './io';
...
function bindSearch(
	...
	const run = async () => {
		...
		const hits = await searchLocation(query, { signal: searchAbort.signal });
		renderHits(hits);
	};
...
export function startProximity(root: HTMLElement): () => void {
	...
	const unitButtons = Array.from(root.querySelectorAll<HTMLButtonElement>('[data-unit]'));
	let statusTimer = 0;
	...
	function fit(force = true) {
		...
		if (persisted.destination) points.push([persisted.destination.lat, persisted.destination.lon]);
		for (const place of persisted.locations) points.push([place.lat, place.lon]);
		if (points.length === 0) {
	...
	function render() {
		...
		for (const btn of unitButtons) {
			btn.setAttribute('aria-pressed', btn.dataset.unit === persisted.unit ? 'true' : 'false');
		}
		...
		if (ranked.length === 0) {
			...
			for (const [index, place] of ranked.entries()) {
				...
				remove.textContent = '×';
				remove.addEventListener('click', () => {
					persisted.locations = persisted.locations.filter((item) => item.id !== place.id);
					render();
				});
		...
		for (const [index, place] of ranked.entries()) {
			const kmLabel = Number.isFinite(place.km) ? formatDistance(place.km, persisted.unit) : '';
			L.marker([place.lat, place.lon], { icon: locIcon(index + 1), zIndexOffset: 400 })
		...
		hint.textContent = dest ? 'Click the map to add a location' : 'Click the map to set a destination';
	...
	function applyFile(data: ProximityFile) {
		persisted.destination = data.destination
			...
			: null;
		persisted.locations = data.locations.map((node) => ({ id: crypto.randomUUID(), ...node }));
		render();
		fit(true);
	}
	...
	function setDestination(place: Place) {
		...
		persisted.locations = persisted.locations.filter((item) => !samePlace(item, place));
		render();
		fit();
	}

	function addLocation(place: Place) {
		if (persisted.destination && samePlace(persisted.destination, place)) return;
	...
	bindSearch(root, destInput, destResults, destForm, setDestination, session.signal);
		...
		() => {
			...
			navigator.geolocation.getCurrentPosition(
				async (pos) => {
					...
					setDestination({ id: crypto.randomUUID(), name: name || 'My location', lat, lon });
	...
	importBtn.addEventListener('click', () => importFile.click(), { signal: session.signal });
