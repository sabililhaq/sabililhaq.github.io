...
describe("map lab", () => {
  it("adds a /map page using the shared layout pieces", () => {
    ...
    expect(source).toMatch(
      /data-map-mode="proximity"[^>]*aria-selected="true"/,
    );
    ...
    expect(source).toMatch(
      /html\.map-lab \.map-mode-bar[\s\S]*position: absolute/,
    );
    expect(source).toMatch(/html\.map-lab \.map-mode-bar[\s\S]*left: 50%/);
    expect(source).toMatch(
      /html\.map-lab \.map-mode-bar[\s\S]*pointer-events: none/,
    );
    expect(source).not.toMatch(
      /html\.map-lab \.map-mode-bar[\s\S]*flex-shrink: 0/,
    );
    expect(source).toMatch(
  ...
  it("links Map from the labs page as an internal lab", () => {
    ...
    expect(lab!.description).toBe(
      "Rank places by distance, or style a map poster.",
    );
